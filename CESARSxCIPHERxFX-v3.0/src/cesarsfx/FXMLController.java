...
public class FXMLController {
	...
	public Media media;
	private boolean aOn=false, rOn=false, check=true, valid, inProgress;
	...
	private static final int faktor = 4000;
		...
		@FXML protected void lautsprecherKlick(ActionEvent event) {
			...
			if (mplayer != null) {
				...
				if (mplayer.getVolume() == 0) {
					...
					Image bild = new Image(getClass().getResource("/icon/mute.gif").toExternalForm());
					symbol.setImage(bild);	
				}
				else {
					//sonst auf 0 setzen
					mplayer.setVolume(0);
					Image bild = new Image(getClass().getResource("/icon/mute_off.gif").toExternalForm());
					symbol.setImage(bild);	
				}
	...
	@FXML public void setAudio() {
		media = new Media(getClass().getResource("/sounds/theromanleginary.mp3").toExternalForm());
	...
	@FXML private void random(ActionEvent event) {
		double math = java.lang.Math.random() * faktor;
		int zufall = (int) math;
		eingabeA.setText(Integer.toString(zufall));
		aOn = true;
		translate(event);
